...
%XXXXXXXXXXXXXXXXXXXXXXX****¡¡REVISADO!!****XXXXXXXXXXXXXXXXXXXXXXXX           
        ...
        \subsection{Gráficas} \label{sec:queesunagrafica}
            ...
            Con lo comentado en los párrafos anteriores, decimos que una \textit{gráfica} \index{Gráfica} $G$ es un par ordenado $(V(G), E(G))$ que consiste de un conjunto $V(G)$ de \textit{vértices} \index{Vértice! de una gráfica} y un conjunto $E(G)$ de \textit{aristas}\index{Arista! de una gráfica} (ajeno con $V(G)$); junto con una \textit{función de incidencia} \index{Función de incidencia! de una gráfica }$\psi_{G} \colon E(G) \rightarrow \binom{V(G)}{1} \cup \binom{V(G)}{2}$ que asocia a cada arista de $G$ un par no ordenado de vértices de $G$ (no necesariamente distintos).Véase el ejemplo \ref{ejem:def}.
